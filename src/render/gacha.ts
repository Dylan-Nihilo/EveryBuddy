...
function getPhaseTimings(tier: number): PhaseTimings {
...
function drawCanvas(io: PromptIO, lines: string[]): void {
...
function redrawCanvas(io: PromptIO, lines: string[]): void {
  const padded = padCanvasLines(lines);
...
function padCanvasLines(lines: string[]): string[] {
  const result: string[] = [];
...
function renderParticleFrames(tier: number): string[][] {
  const chars = [".", "·", "*", "✦", "★"];
  ...
  for (let step = 0; step < frameCount; step++) {
    ...
    for (let row = 0; row < CANVAS_HEIGHT; row++) {
      ...
      const chars: string[] = new Array(CANVAS_WIDTH).fill(" ");
      ...
      if (offset > 0 && center - offset >= 0) chars[center - offset] = particle;
      if (offset > 0 && center + offset < CANVAS_WIDTH) chars[center + offset] = particle;
      if (progress > 0.5) chars[center] = particle;
...
function renderRarityFlash(bones: CompanionBones): string[][] {
  const { rarity } = bones;
...
function renderRevealSprite(bones: CompanionBones): string[] {
  const species = SPECIES[bones.species];
  const eye = EYES[bones.eye];
  if (!species?.frames[0] || !eye) return emptyCanvas();
...
function renderRevealWithName(bones: CompanionBones, soul: CompanionSoul, nameProgress: number): string[] {
  const species = SPECIES[bones.species];
  const eye = EYES[bones.eye];
  if (!species?.frames[0] || !eye) return emptyCanvas();
...
export async function playGachaAnimation(options: GachaAnimationOptions): Promise<void> {
