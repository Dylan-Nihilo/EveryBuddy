...
import { generateStats, type RNG } from "./stats.js";
...
export function pickRandom<T>(rng: RNG, values: T[]): T {
  if (values.length === 0) {
...
export function pickWeighted<T extends { weight: number }>(rng: RNG, values: T[]): T {
  if (values.length === 0) {
