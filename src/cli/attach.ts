...
export async function runAttachCommand(options: AttachCommandOptions): Promise<AttachContext | null> {
  ...
  let candidateSidecarPaneIds = await getEveryBuddySidecarPaneIds(
    tmux,
    windowId,
    sidecarStillAlive ? existingSidecarPaneId : undefined,
  );

  if (candidateSidecarPaneIds.length === 0) {
    const windowWidth = await tmux.currentWindowWidth(windowId).catch(() => 120);
    const createdPaneId = await tmux.splitWindow(
      windowId,
      buildSidecarCommand(windowId, targetPaneId),
      resolveSidecarWidth(windowWidth),
    );
  ...
  if (!options.quiet) {
    process.stdout.write(
      `${created ? "Attached" : "Updated"} EveryBuddy in ${windowId} with sidecar ${sidecarPaneId}.\n`,
    );
  }
...
async function getEveryBuddySidecarPaneIds(
  tmux: TmuxClient,
  windowId: string,
