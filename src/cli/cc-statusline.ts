...
import { padDisplayWidth, visibleLength } from "../render/layout.js";
...
export async function runCCStatusLineCommand(): Promise<void> {
  ...
  const [session, companion, config] = await Promise.all([
    ...
    resolveBuddyConfig().catch(() => ({ language: "zh" as BuddyLanguage })),
  ]);
...
export function renderCompanionStatusLine(
  companion: CompanionRecord,
  session: CCSessionJSON | null,
...
export function renderEmptyStatusLine(session: CCSessionJSON | null): string {
  const rightLines = [
...
export function composeSideBySide(spriteLines: string[], rightLines: string[], centerRight = false): string {
...
export function buildInfoBar(session: CCSessionJSON | null, language: BuddyLanguage): string {
  if (!session) {
    return "";
  }
...
export function parseSessionJSON(raw: string): CCSessionJSON | null {
  if (!raw.trim()) {
    return null;
  }

  try {
    return JSON.parse(raw) as CCSessionJSON;
  } catch {
    return null;
  }
}

function readStdinJSON(): Promise<CCSessionJSON | null> {
