...
import { PROVIDER_DEFAULTS, resolveBuddyConfig, resolveUserId, updateBuddyConfigFile, } from "../storage/config.js";
import { createConsoleIO } from "./io.js";
import { detectTmuxInstallStatus, runInstallTmuxCommand, } from "./install.js";
const STAGE_DELAY_MS = 180;
const SPINNER_FRAMES = ["·  ", "·· ", "···", " ··"];
export async function runBuddyHomeCommand(options = {}) {
...
export async function runSetupCommand(options = {}) {
    ...
    const installFlow = options.installFlow ?? ((params) => runInstallTmuxCommand(params));
...
export async function getBuddyHomeOutput(options) {
    ...
    const tamperWarning = integrity && !integrity.valid
...
async function ensureRuntimeConfig(config, io) {
    if (config.apiKey) {
        return;
    }
    if (!io.isInteractive) {
...
async function playBonesReveal(scene, language, io, sleep) {
    ...
    await sleep(STAGE_DELAY_MS);
    io.writeLine(`${colorize(text.rarityLabel.toLowerCase(), scene.bones.rarity.color)} ${localizeRarityName(scene.bones.rarity.name, language)} ${scene.bones.rarity.stars}`);
    await sleep(STAGE_DELAY_MS);
    io.writeLine(`${colorize(text.speciesLabel.toLowerCase(), scene.bones.color.accent)} ${localizeSpeciesName(scene.bones.species, species?.name ?? scene.bones.species, language)}`);
    io.writeLine(`${colorize(text.eyesLabel.toLowerCase(), scene.bones.color.accent)} ${localizeEyeLabel(scene.bones.eye, eye?.label ?? scene.bones.eye, language)}`);
    io.writeLine(`${colorize(text.hatLabel.toLowerCase(), scene.bones.color.accent)} ${localizeHatLabel(scene.bones.hat, HATS[scene.bones.hat]?.label ?? scene.bones.hat, language)}`);
    await sleep(STAGE_DELAY_MS);
    if (sprite.length > 0) {
        io.writeLine("");
    ...
    io.writeLine("");
    io.writeLine(dim(`${text.peakMarker} ${localizeStatName(peakStat, language)} ${peakValue} · ${text.dumpMarker} ${localizeStatName(dumpStat, language)} ${dumpValue}`));
    io.writeLine("");
}
async function imprintSoul(scene, soul, language, io, sleep) {
    const text = uiText(language);
...
async function playFinalReveal(record, storageDir, language, io, sleep) {
    const text = uiText(language);
    io.writeLine("");
...
async function runWithSpinner(io, label, task) {
    if (!io.supportsAnsi) {
        return task();
    }
    let frame = 0;
...
function resolveSetupUserId(cliUserId, existing) {
    if (cliUserId?.trim()) {
        return resolveUserId(cliUserId);
    }
    return existing?.userId ?? resolveUserId();
}
function cloneSoul(soul) {
    return {
        name: soul.name,
